...
const MusicPage = () => {
  ...
  const checkAuthStatus = async () => {
    try {
      ...
      // Check if user has Spotify connected by trying to fetch playlists
      ...
      if (response.ok) {
        ...
        // User not connected to Spotify yet
  ...
  const loginWithSpotify = () => {
    // Pass the Tunr auth token to link Spotify to the logged-in user
    const token = localStorage.getItem('token');
    const url = token
  ...
  // Login screen - ONLY show if NOT authenticated
  if (!isAuthenticated) {
    return (
      <PageContainer>
        <LoginContainer>
